...
const downloadPtau = (dir) => {
  if (!fs.existsSync(`${dir}/${ptau}`)) {
    ...
    execSync(
      `cd ${dir} && wget https://hermez.s3-eu-west-1.amazonaws.com/${ptau}`,
      ENOBUFSExecOpt
    )
  }
}
...
const generateR1csWitnessGen = (dir) => {
  circuits.forEach((x) => {
    ...
    execSync(
      `cd ${circuitDir} && PATH=$HOME/.cargo/bin:$PATH circom --r1cs --c ${x}.circom`
    )

    console.log(`Building witness generator and r1cs of "${x}"...`)
    execSync(`cd ${circuitDir}/${x}_cpp && make`)

    const ifWrap = (simlink, makeSimlink) =>
      `if [ ! -f ${simlink} ]; then ${makeSimlink}; fi`

    execSync(
      ifWrap(`${dir}/${x}`, `ln -s ${circuitDir}/${x}_cpp/${x} ${dir}/${x}`)
    )
    execSync(
      ifWrap(
        `${dir}/${x}.dat`,
        `ln -s ${circuitDir}/${x}_cpp/${x}.dat ${dir}/${x}.dat`
      )
    )
    execSync(
      ifWrap(
        `${dir}/${x}.r1cs`,
        `ln -s ${circuitDir}/${x}.r1cs ${dir}/${x}.r1cs`
      )
    )
  })
}
