...
import { genRandomSalt, hash3, hash5, IncrementalQuinTree } from 'maci-crypto'
...
describe('Process messages and tally', () => {
    ...
    describe('Publish messages', () => {
      ...
      it('should have same message root after publishing message', async () => {
        ...
        for (const [i, voter] of S.voters.entries()) {
          ...
          S.maciStatePoll.publishMessage(message, ephemeralKeypair.pubKey)
        }
      ...
      it('should be able to verify process messages proof w/ public input created from MACIState circuit inputs and other inputs from MACI Poll', async () => {
        ...
        fs.writeFileSync(
          `${testDir}/pm-input.json`,
          JSON.stringify(pmCircuitInputs)
        )

        // generate pm witness from the pm circuit input
        await exec(
          `cd ${testDir} && ./processMessages_test pm-input.json processMessages_test.wtns`
        )

        // write public input hash from circuit inputs to file
        const pmPublicInputHash = BigInt(pmCircuitInputs.inputHash)
        fs.writeFileSync(
          `${testDir}/pm-public-code.json`,
          JSON.stringify([pmPublicInputHash.toString()])
        )

        // generate proof
        await exec(
          `cd ${testDir} && snarkjs groth16 prove processMessages_test.zkey processMessages_test.wtns pm-proof.json pm-public.json`
        )
        ...
        const pollProcessorAndTallyer = await B.PollProcessorAndTallyer.deploy(
          verifier.address
        )

        const snarkJsPmProof = JSON.parse(
          fs.readFileSync(`${testDir}/pm-proof.json`)
        ) as SnarkJsProof
        const pmProof = buildMaciProof(snarkJsPmProof)

        const snarkJsPmVKey = JSON.parse(
          fs.readFileSync(`${testDir}/processMessages_test.vkey`)
        ) as SnarkJsVKey
        const pmVk = buildMaciVerifyingKey(snarkJsPmVKey)

        const snarkJsTallyVKey = JSON.parse(
          fs.readFileSync(`${testDir}/tallyVotes_test.vkey`)
        ) as SnarkJsVKey
        ...
        // write pm circuit inputs to file
        fs.writeFileSync(
          `${testDir}/tally-input-code.json`,
          JSON.stringify(tallyCircuitInputs)
        )

        // generate pm witness from the pm circuit input
        await exec(
          `cd ${testDir} && ./tallyVotes_test tally-input-code.json tallyVotes_test.wtns`
        )
        ...
        // generate proof
        await exec(
          `cd ${testDir} && snarkjs groth16 prove tallyVotes_test.zkey tallyVotes_test.wtns tally-proof.json tally-public.json`
        )

        const snarkJsTallyProof = JSON.parse(
          fs.readFileSync(`${testDir}/tally-proof.json`)
        ) as SnarkJsProof
  ...
  describe('batchProcessMessage', () => {
    ...
    it('should be that initial sbCommitment should match the one generated from empty Ballot', async () => {
      ...
      const ballot = Ballot.genBlankBallot(
        numVoteOptions,
