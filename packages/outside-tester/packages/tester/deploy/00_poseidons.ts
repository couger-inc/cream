...
const genPoseidonABI = (name: string, numArgs: number) => {
  const abi: WithName[] = circomlibjs.poseidonContract.generateABI(numArgs)
  abi.forEach((x) => {
    x.name = name
  })
  return abi
...
const func: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
  ...
  const contract: ArtifactData = {
    ...
    bytecode: circomlibjs.poseidonContract.createCode(2),
  }
  ...
  for (const numArgs of [2, 3, 4, 5]) {
    ...
    const bytecode = circomlibjs.poseidonContract.createCode(numArgs)
