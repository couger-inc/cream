...
const generateVerifier = (
    ...
    let template = fs.readFileSync(
        path.join(__dirname, '../ts/verifier_groth16.sol'), 'utf-8')

    const vkalpha1_str = `${vk.vk_alpha_1[0].toString()},` +
        `${vk.vk_alpha_1[1].toString()}`;
    template = template.replace("<%vk_alpha1%>", vkalpha1_str);

    const vkbeta2_str = `[${vk.vk_beta_2[0][1].toString()},` +
    ...
    const vkgamma2_str = `[${vk.vk_gamma_2[0][1].toString()},` +
    ...
    const vkdelta2_str = `[${vk.vk_delta_2[0][1].toString()},` +
    ...
    template = template.replace(/<%vk_input_length%>/g, (vk.IC.length - 1).toString());
    template = template.replace("<%vk_ic_length%>", vk.IC.length.toString());
    let vi = "";
    for (let i = 0; i < vk.IC.length; i++) {
        if (vi != "") vi = vi + "        ";
        vi = vi + `vk.IC[${i}] = Pairing.G1Point(${vk.IC[i][0].toString()},` +
            `${vk.IC[i][1].toString()});\n`;
    }
